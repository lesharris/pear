...
const themeApi: DocsApiItem[] = [
  {
    name: 'PThemeSwitcher',
    type: 'component',
    description: 'Renders a light/dark theme toggle button.',
  },
  {
    name: 'useTheme',
    type: "() => { theme: Ref<'light' | 'dark'>, toggleTheme: () => void }",
    description: 'Reads, writes, and toggles the current theme.',
  },
  {
    name: 'storage',
    type: "'pear-theme'",
    description: 'Theme preference is persisted to localStorage when available.',
  },
  {
    name: 'document attribute',
    type: 'data-theme',
    description: 'The current theme is written to documentElement.',
  },
  {
    name: 'default',
    type: 'prefers-color-scheme',
    description: "When storage is empty, Pear starts with the user's OS preference.",
  },
]
</script>
...
<template>
  <section id="overview" data-section class="docs-section">
    <DocsIntroCard name="PThemeSwitcher">
      <code>PThemeSwitcher</code> gives users a light/dark toggle. Use <code>useTheme</code> when
      you want to read or set the same value yourself.
    </DocsIntroCard>
  </section>
