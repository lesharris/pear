...
export const docsMeta = {
  ...
  description: 'Text input examples for Pear, including field wrappers and common input states.',
  tocItems: [
...
const inputProps: DocsApiItem[] = [
  {
    ...
    type: 'string | number',
    description:
      'Input value. Number inputs write numbers, and empty number inputs write an empty string.',
  },
  {
    name: 'type',
    type: "'text' | 'email' | 'number' | 'password' | 'tel' | 'url' | 'search'",
    default: "'text'",
    description: 'Text-like native input type rendered by PInput.',
  },
  {
    name: 'disabled',
    type: 'boolean',
    default: 'false',
    description: 'Disables the input. Inherits from PField when present.',
  },
...
<template>
  <section id="overview" data-section class="docs-section">
    <DocsIntroCard name="PInput">
      <code>PInput</code> covers the usual text-like inputs with a plain Vue <code>v-model</code>,
      plus Pico's normal input styling.
    </DocsIntroCard>
  </section>
    ...
    <p-card>
      ...
      <AppStack>
        <DocsExample :code="nameInputCode">
          <p-field label="Name" helper="Please enter your name.">
            <p-input v-model="name" name="name" placeholder="Enter your name" autocomplete="name" />
          </p-field>
        </DocsExample>
        ...
        <DocsExample :code="searchInputCode">
          <p-field label="Search" helper="Pico keeps search inputs looking like search inputs.">
            <p-input v-model="search" type="search" name="search" placeholder="Search..." />
          </p-field>
        </DocsExample>

        <DocsExample :code="numberInputCode">
          <p-field label="Age" helper="Number input using the same PInput component.">
            <p-input v-model="age" type="number" name="age" placeholder="42" min="0" max="120" />
          </p-field>
        </DocsExample>
