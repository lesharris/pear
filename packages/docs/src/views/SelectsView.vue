...
const selectProps: DocsApiItem[] = [
  {
    ...
    description: 'Selected value, or selected values when multiple is true.',
  },
  {
    name: 'options',
    type: 'Array<string | number | object>',
    default: '[]',
    description: 'Options rendered when no default slot is provided.',
  },
  {
    name: 'optionLabel',
    type: 'string',
    default: "'label'",
    description: 'Object key used for option text.',
  },
  {
    name: 'optionValue',
    type: 'string',
    default: "'value'",
    description: 'Object key used for option value.',
  },
  {
    name: 'placeholder',
    type: 'string',
    description: 'Disabled placeholder option for single selects.',
  },
  {
    name: 'multiple',
    type: 'boolean',
    default: 'false',
    description: 'Enables native multiple selection.',
  },
  { name: 'size', type: 'number', description: 'Native visible row count.' },
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Disables the select. Inherits from PField when present.',
  },
  {
    name: 'invalid',
    type: 'boolean',
...
const selectSlots: DocsApiItem[] = [
  {
    ...
    type: 'slot',
    description:
      'Optional custom option markup. When omitted, options are rendered from the options prop.',
  },
]
...
const selectEvents: DocsApiItem[] = [
  {
    ...
    type: 'string | number | Array<string | number>',
    description: 'Emitted by v-model when the native select changes.',
  },
]
</script>
...
<template>
  <section id="overview" data-section class="docs-section">
    <DocsIntroCard name="PSelect">
      <code>PSelect</code> is for native select fields: simple lists, object options, multiple
      selection, and normal form states. For a little action menu, reach for <code>PDropdown</code>.
    </DocsIntroCard>
  </section>
  ...
  <section id="states" data-section class="docs-section">
    <p-card>
      ...
      <DocsExample :code="stateSelectCode" preview-overflow="visible">
        <AppStack>
          <p-field label="Disabled select" disabled>
            <p-select v-model="disabledCuisine" name="disabled-cuisine" :options="cuisines" />
          </p-field>
