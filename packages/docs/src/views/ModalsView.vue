...
const modalProps: DocsApiItem[] = [
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Controls whether the native dialog is open.',
  },
  {
...
const modalSlots: DocsApiItem[] = [
  {
    ...
    type: 'slot',
    description:
      'Header content. A close button is rendered automatically when this slot is present.',
  },
  {
    name: 'default',
    type: '{ close: () => void }',
    description: 'Modal body content with a close slot prop.',
  },
  {
    name: 'footer',
    type: '{ close: () => void }',
    description: 'Footer actions with a close slot prop.',
  },
]
...
const modalEvents: DocsApiItem[] = [
  {
    ...
    type: 'boolean',
    description: 'Emitted by v-model when the modal closes.',
  },
...
const modalProviderApi: DocsApiItem[] = [
  {
    name: 'PModalProvider',
    type: 'component',
    description: 'Provides programmatic modal context to child components.',
  },
  {
    name: 'useModal().open',
    type: '(options: PModalOptions) => Promise<unknown>',
    description: "Opens a provider modal and resolves with the clicked action's return value.",
  },
  {
    name: 'useModal().close',
    type: '(result?: unknown) => void',
    description: 'Closes the provider modal and optionally resolves it with a value.',
  },
]
</script>
...
<template>
  <section id="modal-overview" data-section class="docs-section">
    <DocsIntroCard name="PModal">
      <code>PModal</code> uses the native dialog element with <code>v-model</code>, footer close
      helpers, and an after-close event.
    </DocsIntroCard>
  </section>
  ...
  <section id="vmodel-modal" data-section class="docs-section">
    <p-card>
      ...
      <AppStack>
        <p>This modal is controlled directly by local component state.</p>

        <DocsExample :code="vmodelModalCode">
          <p-button @click="localModalOpen = true"> Open v-model modal </p-button>
          ...
          <p-modal v-model="localModalOpen">
            ...
            <p>This modal is opened and closed with a local v-model binding.</p>
  ...
  <section id="backdrop" data-section class="docs-section">
    <p-card>
      ...
      <DocsExample :code="backdropModalCode">
        <p-button variant="secondary" @click="pinnedModalOpen = true"> Open pinned modal </p-button>
  ...
  <section id="provider-overview" data-section class="docs-section">
    <DocsIntroCard name="PModalProvider">
      Wrap your app once with <code>PModalProvider</code>, then call <code>useModal()</code> from
      child views or components.
    </DocsIntroCard>
  </section>
    ...
    <p-card>
      ...
      <AppStack>
        <p>
          ...
          <code>modal.open()</code> returns a promise, you can await the user's choice.
        </p>

        <DocsExample :code="programmaticModalCode" language="ts">
          <AppStack gap="0.75rem">
            <p-button @click="openProgrammaticModal"> Open programmatic modal </p-button>
