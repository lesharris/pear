...
export const docsMeta = {
  ...
  description: 'Standalone, exclusive, named, and button-style accordion examples for Pear.',
  tocItems: [
...
const accordionProps: DocsApiItem[] = [
  {
    name: 'summary',
    type: 'string',
    description: 'Text shown in the details summary when no summary slot is provided.',
  },
  {
    name: 'open',
    type: 'boolean',
    default: 'false',
    description: 'Initial native details open state. Can be controlled with v-model:open.',
  },
  {
    name: 'name',
    type: 'string',
    description: 'Native details group name. Inherits from PAccordionGroup when present.',
  },
  {
    name: 'button',
    type: 'boolean',
    default: 'false',
    description: "Applies Pico's button-style summary pattern.",
  },
  {
    name: 'variant',
    type: "'secondary' | 'contrast'",
    description: 'Button summary variant when button is true.',
  },
  {
    name: 'outline',
    type: 'boolean',
    default: 'false',
    description: 'Button summary outline style when button is true.',
  },
]
...
const accordionGroupProps: DocsApiItem[] = [
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Provides a shared details name so only one child accordion opens at a time.',
  },
  {
    name: 'name',
    type: 'string',
    description:
      'Stable native details group name. Generated when exclusive is true and no name is provided.',
  },
]
...
const accordionEvents: DocsApiItem[] = [
  {
    name: 'update:open',
    type: 'boolean',
    description: 'Emitted when the native details element opens or closes.',
  },
]
</script>
...
<template>
  <section id="accordion-overview" data-section class="docs-section">
    <DocsIntroCard name="PAccordion">
      <code>PAccordion</code> uses native details and summary markup, with a couple of Pear extras
      for button-style summaries and custom headings.
    </DocsIntroCard>
  </section>
  ...
  <section id="standalone" data-section class="docs-section">
    <p-card>
      ...
      <DocsExample :code="standaloneCode">
        <p-accordion summary="What is Pico CSS?" v-model:open="isPicoOpen">
          <p>
            Pico gives semantic HTML a clean default look. Pear keeps that HTML shape and makes the
            Vue bits a little easier.
          </p>
        </p-accordion>

        <p-accordion summary="Can the summary use a slot?">
          <p>
            Yes. The simple case uses the summary prop, but richer headings can use the named
            summary slot.
          </p>
        </p-accordion>
        ...
        <p-accordion>
          ...
          <p>The summary slot is useful when the heading needs markup.</p>
        </p-accordion>
      </DocsExample>
    </p-card>
  </section>
  ...
  <section id="group-overview" data-section class="docs-section">
    <DocsIntroCard name="PAccordionGroup">
      <code>PAccordionGroup</code> links child accordions together when only one panel should stay
      open.
    </DocsIntroCard>
  </section>
  ...
  <section id="exclusive-group" data-section class="docs-section">
    <p-card>
      ...
      <AppStack>
        <p>These share a generated native details name, so opening one closes the others.</p>
  ...
  <section id="named-group" data-section class="docs-section">
    <p-card>
      ...
      <AppStack>
        <p>
          Give the group a name when you want the native details grouping to stay stable across
          renders.
        </p>
