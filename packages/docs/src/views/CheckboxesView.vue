...
export const docsMeta = {
  ...
  description: 'Checkbox, switch, checkbox group, and radio group examples for Pear.',
  tocItems: [
...
const checkboxProps: DocsApiItem[] = [
  {
    ...
    description: 'Checked value or selected values array.',
  },
  {
    name: 'value',
    type: 'string | number | boolean',
    default: 'true',
    description: 'Value written when checked, or array item used in group mode.',
  },
  {
    name: 'uncheckedValue',
    type: 'string | number | boolean',
    default: 'false',
    description: 'Value written when unchecked outside array mode.',
  },
  {
    name: 'switch',
    type: 'boolean',
    default: 'false',
    description: 'Renders the checkbox with role switch for Pico switch styling.',
  },
  {
    name: 'indeterminate',
    type: 'boolean',
    default: 'false',
    description: 'Sets the native indeterminate visual state.',
  },
  {
    name: 'disabled',
    type: 'boolean',
    default: 'false',
    description: 'Disables the checkbox. Inherits from PField when present.',
  },
  {
    name: 'invalid',
    type: 'boolean',
...
const radioProps: DocsApiItem[] = [
  { name: 'v-model', type: 'string | number | boolean', description: 'Selected radio value.' },
  {
    name: 'value',
    type: 'string | number | boolean',
    description: 'Value written when this radio is checked.',
  },
  {
    name: 'disabled',
    type: 'boolean',
    default: 'false',
    description: 'Disables the radio. Inherits from PField when present.',
  },
  {
    name: 'invalid',
    type: 'boolean',
...
const groupProps: DocsApiItem[] = [
  {
    name: 'v-model',
    type: 'PChoiceValue[] | PChoiceValue',
    description: 'Selected checkbox values, or the selected radio value.',
  },
  {
    name: 'options',
    type: 'PChoiceOption[]',
    description: 'Options with label, value, and optional disabled state.',
  },
  {
    name: 'name',
    type: 'string',
    description: 'Native input name. Pear creates one when omitted.',
  },
  ...
  { name: 'helper', type: 'string', description: 'Helper text below the options.' },
  {
    name: 'error',
    type: 'string',
    description: 'Error text below the options. Also marks the group invalid.',
  },
  { name: 'disabled', type: 'boolean', default: 'false', description: 'Disables the whole group.' },
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Marks the whole group invalid.',
  },
  {
    name: 'horizontal',
    type: 'boolean',
  ...
  },
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'PCheckboxGroup only. Renders options as switches.',
  },
]
...
const choiceSlots: DocsApiItem[] = [
  {
    name: 'default',
    type: 'slot',
    description: 'Label content rendered next to the native checkbox or radio.',
  },
  {
    name: 'option',
    type: 'slot',
    description: 'PCheckboxGroup and PRadioGroup option label slot.',
  },
]
...
const choiceEvents: DocsApiItem[] = [
  {
    ...
    type: 'value or value[]',
    description: 'Emitted by v-model when the native input changes.',
  },
]
</script>
...
<template>
  <section id="checkbox-overview" data-section class="docs-section">
    <DocsIntroCard name="PCheckbox">
      <code>PCheckbox</code> keeps native checkbox behavior and adds the bits you usually want in
      Vue: boolean models, array models, switches, and indeterminate state.
    </DocsIntroCard>
  </section>
  ...
  <section id="radio-overview" data-section class="docs-section">
    <DocsIntroCard name="PRadio">
      <code>PRadio</code> is the same idea for radio buttons: native inputs with a tidy scalar
      <code>v-model</code>.
    </DocsIntroCard>
  </section>
