...
const selectedAttachmentNames = computed(() => {
  ...
  return Array.from(attachments.value)
    .map((file) => file.name)
    .join(', ')
})
...
const colorInputProps: DocsApiItem[] = [
  {
    ...
    type: 'string',
    description:
      'Selected color as a native color input string, usually a hex value such as #00dfa2.',
  },
  { name: 'type', type: "'color'", description: 'Renders a native color picker input.' },
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Disables the color picker. Inherits from PField when present.',
  },
  {
    name: 'invalid',
    type: 'boolean',
...
const fileInputProps: DocsApiItem[] = [
  {
    name: 'v-model',
    type: 'FileList | null',
    description: 'Selected files from the native file input.',
  },
  { name: 'type', type: "'file'", description: 'Renders a native file upload input.' },
  {
    name: 'accept',
    type: 'string',
    description: 'Native file type filter passed through as an attribute, such as .pdf,image/*.',
  },
  {
    name: 'multiple',
    type: 'boolean',
    default: 'false',
    description: 'Native multiple file selection passed through as an attribute.',
  },
  {
    name: 'disabled',
    type: 'boolean',
    default: 'false',
    description: 'Disables the file input. Inherits from PField when present.',
  },
  {
    name: 'invalid',
    type: 'boolean',
...
const colorInputEvents: DocsApiItem[] = [
  {
    ...
    type: 'string',
    description: 'Emitted by v-model when the selected color changes.',
  },
]
...
const fileInputEvents: DocsApiItem[] = [
  {
    name: 'update:modelValue',
    type: 'FileList | null',
    description: 'Emitted by v-model when the selected files change.',
  },
]
</script>
...
<template>
  <section id="overview" data-section class="docs-section">
    <DocsIntroCard name="PInput">
      <code>PInput</code> handles the native odds and ends too: color pickers and file inputs, with
      the same field and <code>v-model</code> shape as the text inputs.
    </DocsIntroCard>
  </section>
